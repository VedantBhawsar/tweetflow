...
function LoginContent() {
        ...
        if (callbackError) {
            switch (callbackError) {
                ...
                case "OAuthAccountNotLinked":
                    setError("This email is already associated with another account. Try signing in with a different method.");
                    break;
                default:
    ...
    const handleCredentialsSignIn = async (e: React.FormEvent) => {
        ...
        if (result?.error) {
            setError(result.error === "CredentialsSignin" ? "Invalid email or password." : "An error occurred: " + result.error);
        } else if (result?.ok && result?.url) {
            router.push(searchParams?.get("callbackUrl") || "/dashboard"); // Redirect to dashboard or callbackUrl
        } else if (result?.ok) {
    ...
    const handleOAuthSignIn = (providerId: string) => {
        setLoading(true); // Show loading for OAuth as well
